...
const spring = (k: number, m: number, bounce: number): Spring => ({
  type: "spring",
  stiffness: k,
...
export default function JellyRadio({
  ...
  useEffect(() => {
    ...
    applied.current = at;
    // Scroll updates the value from outside. Animate that the same way as a tap.
    apply(at, false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [at]);
  ...
  return (
    <div
      ...
      style={{
        ["--jr-chip" as string]: chipColor,
        ["--jr-active" as string]: activeColor,
        ["--jr-text" as string]: textColor,
        ["--jr-active-text" as string]: activeTextColor,

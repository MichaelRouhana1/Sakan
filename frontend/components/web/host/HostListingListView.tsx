      ...
      {rows.map((row) => {
        if (row.kind === "local-draft") {
          ...
          return (
            ...
            >
              <View style={[styles.dataCell, styles.listingCol, styles.listingCell]}>
                <View style={styles.thumb}>
                  {cover ? (
                    <Image source={{ uri: cover }} style={StyleSheet.absoluteFill} contentFit="cover" />
                  ) : (
                    <View style={styles.thumbPlaceholder}>
                      <Ionicons name="image-outline" size={16} color={Skoun.color.inkFaint} />
                    </View>
                  )}
                </View>
              ...
              <Text style={[styles.dataCell, styles.typeCol]} numberOfLines={1}>
                {typeLabel}
              </Text>
              <Text style={[styles.dataCell, styles.locationCol]} numberOfLines={1}>
        ...
        const title =
          listing.title?.trim() ||
          `Your listing started ${new Date(listing.createdAt).toLocaleDateString("en-US", {
            month: "long",
            day: "numeric",
            year: "numeric",
          })}`;
